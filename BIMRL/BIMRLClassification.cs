...
namespace BIMRL
{
    public class BIMRLClassification
    {
        ...
        public BIMRLClassification(XbimModel m, BIMRLCommon refBIMRLCommon)
        {
        ...
        public void processClassificationItems()
        {
            ...
            IEnumerable<IfcRelAssociatesClassification> relClasses = _model.InstancesLocal.OfType<IfcRelAssociatesClassification>();
            foreach (IfcRelAssociatesClassification relClass in relClasses)
            {
                if (!(relClass.RelatingClassification is IfcClassificationReference))
                    continue;   // We only deal with IfcClassificationReference here

                IfcClassificationReference classRef = relClass.RelatingClassification as IfcClassificationReference;

                string itemLocation = string.Empty;
                if (classRef.Location != null)
                    itemLocation = classRef.Location;

                if (classRef.ItemReference == null)
                    continue;
                string itemRef = classRef.ItemReference;
                ...
                if (classRef.ReferencedSource == null)
                {
                    ...
                    //string[] classifVald = new string[2];
                    //classifVald[0] = "Default Source";
                    //refSource = classifVald[0];
                    //classifVald[1] = string.Empty;
                    //Tuple<string, string> classifd = new Tuple<string,string>(refName, refEdition);
                    //classifList.Add(classifd, classifVald);
                }
                else
                {
                    refName = classRef.ReferencedSource.Name;
                    refEdition = classRef.ReferencedSource.Edition;
                    refSource = classRef.ReferencedSource.Source;
                    if (classRef.ReferencedSource.EditionDate != null)
                        refEdDate =  classRef.ReferencedSource.EditionDate.DayComponent.ToString() + "-" 
                                    + classRef.ReferencedSource.EditionDate.MonthComponent.ToString() + "-"
                                    + classRef.ReferencedSource.EditionDate.YearComponent.ToString();
                }
