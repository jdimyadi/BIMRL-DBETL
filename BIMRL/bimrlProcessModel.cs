...
namespace BIMRL
{
    public class bimrlProcessModel
    {
        ...
        public bimrlProcessModel(XbimModel model, bool update)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(proj.LongName))
                    projLName = proj.Name + " - Federated";
                else
                    projLName = proj.LongName;

                currStep = "Getting Federated ID from BIMRL_FEDERATEDMODEL - Project name and longname: " + proj.Name + "; " + proj.LongName;
                FedIDStatus stat = DBOperation.getFederatedID(projLName, proj.Name, out _FederatedID);
                ...
                if (model.IsFederation)
                {
                    ...
                    foreach (XbimModel m in model.AllModels)
                    {
        ...
        private void doModel(XbimModel m)
        {
            string projName = m.IfcProject.Name;
            string SqlStmt;
            string currStep = string.Empty;
            ...
            try
            {
                ...
                SqlStmt = "Insert into BIMRL_ModelInfo_" + _FederatedID.ToString("X4") + " (ModelID, ModelName, Source) Values ("
                            + _ModelID + ",'" + projName + "','" + m.DatabaseName + "')";
                currStep = SqlStmt;
