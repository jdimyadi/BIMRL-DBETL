...
namespace BIMRL
{
    public class BIMRLRelationship
    {
        ...
        public BIMRLRelationship(XbimModel m, BIMRLCommon refBIMRLCommon)
        {
            _model = m;
        ...
        private void processRelAggregation()
        {
            ...
            IEnumerable<IfcRelAggregates> rels = _model.InstancesLocal.OfType<IfcRelAggregates>();
            foreach (IfcRelAggregates aggr in rels)
            {
                ...
                IEnumerable<IfcObjectDefinition> relObjects = aggr.RelatedObjects;
                foreach (IfcObjectDefinition relObj in relObjects)
                {
        ...
        private void processRelConnections()
        {
            ...
            IEnumerable<IfcRelConnectsPortToElement> ptes = _model.InstancesLocal.OfType<IfcRelConnectsPortToElement>();
            foreach (IfcRelConnectsPortToElement pte in ptes)
            {
            ...
            IEnumerable<IfcRelConnects> rels = _model.InstancesLocal.OfType<IfcRelConnects>(true).Where
                    (re => !(re is IfcRelConnectsPortToElement || re is IfcRelContainedInSpatialStructure || re is IfcRelConnectsStructuralActivity || re is IfcRelConnectsStructuralMember
                            || re is IfcRelFillsElement || re is IfcRelVoidsElement || re is IfcRelSequence || re is IfcRelSpaceBoundary));
            foreach (IfcRelConnects conn in rels)
            {
                if (conn is IfcRelConnectsPathElements)
                {
                    IfcRelConnectsPathElements connPE = conn as IfcRelConnectsPathElements;
                ...
                else if (conn is IfcRelConnectsWithRealizingElements)
                {
                    IfcRelConnectsWithRealizingElements connPE = conn as IfcRelConnectsWithRealizingElements;
                    if (_refBIMRLCommon.getLineNoFromMapping(connPE.RelatingElement.GlobalId.ToString()) == null)
                        continue;       // skip "non" element guid in the relationship object

                    //Iterate for each Realizing element. One record for each realizing element
                    foreach (IfcElement realElem in connPE.RealizingElements)
                    {
                        cIngEle.Add(connPE.RelatingElement.GlobalId.ToString());
                        cIngEleTyp.Add(connPE.RelatingElement.GetType().Name.ToUpper());
                        cEdEle.Add(connPE.RelatedElement.GlobalId.ToString());
                ...
                else if (conn is IfcRelConnectsElements)
                {
                    IfcRelConnectsElements connPE = conn as IfcRelConnectsElements;
                    if (_refBIMRLCommon.getLineNoFromMapping(connPE.RelatingElement.GlobalId.ToString()) == null)
                        continue;       // skip "non" element guid in the relationship object
                ...
                else if (conn is IfcRelConnectsPorts)
                {
                    ...
                    IfcRelConnectsPorts connPE = conn as IfcRelConnectsPorts;

                    IfcDistributionPort port1 = connPE.RelatingPort as IfcDistributionPort;
                    IfcDistributionPort port2 = connPE.RelatedPort as IfcDistributionPort;
                ...
                else if (conn is IfcRelCoversSpaces)
                {
                    IfcRelCoversSpaces covS = conn as IfcRelCoversSpaces;
                    if (_refBIMRLCommon.getLineNoFromMapping(covS.RelatedSpace.GlobalId.ToString()) == null)
                        continue;       // skip "non" element guid in the relationship object

                    IEnumerable<IfcCovering> relCovs = covS.RelatedCoverings;
                    foreach (IfcCovering cov in relCovs)
                    {
                        cIngEle.Add(covS.RelatedSpace.GlobalId.ToString());
                        cIngEleTyp.Add(covS.RelatedSpace.GetType().Name.ToUpper());
                ...
                else if (conn is IfcRelCoversBldgElements)
                {
                    IfcRelCoversBldgElements covE = conn as IfcRelCoversBldgElements;
                    if (_refBIMRLCommon.getLineNoFromMapping(covE.RelatingBuildingElement.GlobalId.ToString()) == null)
                        continue;       // skip "non" element guid in the relationship object

                    IEnumerable<IfcCovering> relCovs = covE.RelatedCoverings;
                    foreach (IfcCovering cov in relCovs)
                    {
                ...
                else if (conn is IfcRelFlowControlElements)
                {
                    // Handle Flow Control
                    IfcRelFlowControlElements connPE = conn as IfcRelFlowControlElements;
                    if (_refBIMRLCommon.getLineNoFromMapping(connPE.RelatingFlowElement.GlobalId.ToString()) == null)
                        continue;       // skip "non" element guid in the relationship object

                    foreach (IfcDistributionControlElement dist in connPE.RelatedControlElements)
                    {
                    ...
                    /* New entity in IFC4
                else if (conn is IfcRelInterferesElements)
                {
                    // Handle Interference
                    IfcRelConnectsElements connPE = conn as IfcRelConnectsElements;
                    if (_refBIMRLCommon.getLineNoFromMapping(connPE.RelatingElement.GlobalId.ToString()) == null)
                     ...
                     */
                else if (conn is IfcRelReferencedInSpatialStructure)
                {
                    // Handle referenced by Spatial Structure
                    IfcRelReferencedInSpatialStructure connPE = conn as IfcRelReferencedInSpatialStructure;
                    if (_refBIMRLCommon.getLineNoFromMapping(connPE.RelatingStructure.GlobalId.ToString()) == null)
                        continue;       // skip "non" element guid in the relationship object

                    foreach (IfcProduct elem in connPE.RelatedElements)
                    {
                ...
                else if (conn is IfcRelServicesBuildings)
                {
                    // Handle MEP connections through IfcDistributionPorts (Port will not be captured)
                    IfcRelServicesBuildings connPE = conn as IfcRelServicesBuildings;
                    if (_refBIMRLCommon.getLineNoFromMapping(connPE.RelatingSystem.GlobalId.ToString()) == null)
                        continue;       // skip "non" element guid in the relationship object

                    foreach (IfcSpatialStructureElement bldg in connPE.RelatedBuildings)
                    {
                ...
                else if (conn is IfcRelConnectsStructuralElement)
                {
                    IfcRelConnectsStructuralElement connPE = conn as IfcRelConnectsStructuralElement;
                    if (_refBIMRLCommon.getLineNoFromMapping(connPE.RelatingElement.GlobalId.ToString()) == null)
                        continue;       // skip "non" element guid in the relationship object

                    IfcStructuralMember stru = connPE.RelatedStructuralMember as IfcStructuralMember;
                    cIngEle.Add(connPE.RelatingElement.GlobalId.ToString());
                    cIngEleTyp.Add(connPE.RelatingElement.GetType().Name.ToUpper());
        ...
        private void processRelSpaceBoundary()
        {
            ...
            IEnumerable<IfcRelSpaceBoundary> rels = _model.InstancesLocal.OfType<IfcRelSpaceBoundary>();
            foreach (IfcRelSpaceBoundary spb in rels)
            {
                string spaceGuid = spb.RelatingSpace.GlobalId.ToString();
                if ((_refBIMRLCommon.getLineNoFromMapping(spaceGuid) == null) || (spb.RelatedBuildingElement == null))
                    continue;   // skip relationship that involves "non" element Guids

                if (spb.RelatedBuildingElement is IfcVirtualElement)
                {
                    // We will ignore the virtual element and instead get the conencted space as the related boundary
                    IfcVirtualElement ve = spb.RelatedBuildingElement as IfcVirtualElement;
                    // get the element this IfcVirtualElement is connected to
                    IEnumerable<IfcRelSpaceBoundary> veSpaces = ve.ProvidesBoundaries;
                    foreach (IfcRelSpaceBoundary veSp in veSpaces)
                    {
                        int lineNo;
                        string space2Guid = veSp.RelatingSpace.GlobalId.ToString();
        ...
        private void processRelGroup()
        {
            ...
            IEnumerable<IfcRelAssignsToGroup> rels = _model.InstancesLocal.OfType<IfcRelAssignsToGroup>();
            foreach (IfcRelAssignsToGroup rGr in rels)
            {
                ...
                IEnumerable<IfcObjectDefinition> members = rGr.RelatedObjects;

                foreach (IfcObjectDefinition oDef in members)
                {
        ...
        private void processElemDependency()
        {
            ...
            IEnumerable<IfcRelConnects> rels = _model.InstancesLocal.OfType<IfcRelConnects>(true).Where
                    (re => (re is IfcRelFillsElement || re is IfcRelVoidsElement));
            foreach (IfcRelConnects conn in rels)
            {
                if (conn is IfcRelVoidsElement)
                {
                    IfcRelVoidsElement connPE = conn as IfcRelVoidsElement;
                ...
                else if (conn is IfcRelProjectsElement)
                {
                    IfcRelProjectsElement connPE = conn as IfcRelProjectsElement;
                    if ((_refBIMRLCommon.getLineNoFromMapping(connPE.RelatingElement.GlobalId.ToString()) == null)
                ...
                else if (conn is IfcRelFillsElement)
                {
                    IfcRelFillsElement connPE = conn as IfcRelFillsElement;
